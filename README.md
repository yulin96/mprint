...
`elements` 按数组顺序从下到上渲染，支持 `type: 'image'` 和 `type: 'text'`。图片使用毫米尺寸、`fit` 和 `rotate` 直接渲染，透明 PNG 保持透明；单个图片元素的宽高不能超过 1200mm。旧版 `images`、`texts` 请求仍可使用，并保持图片在下、文字在上的原有顺序，但不能与 `elements` 同时提供。
